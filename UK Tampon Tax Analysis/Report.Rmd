...
For the most part, the ACF graph indicates that the model has accounted for most of the autocorrelation. The plot of residuals vs fitted also suggests that the errors are likely homoscedastic -- aside from a few outliers, the residuals do not appear to have a changing variance. The standard errors can be trusted as a result.

# Robustness Check

Recall that the key assumption underlying the methodology of this analysis is that the observed price drop is attributable solely to the tax change after accounting for the impact of time. To test whether another event that suddenly influences prices may have occurred at the same time as the abolition of the tampon tax, the same methodology used for tampons is applied to several related goods — tissues, toilet rolls, baby wipes, and razor cartridges^[@neidle2022 look at a total of 13 other goods. For brevity, only four are analyzed in this report.]. 

The results are reported in the table below.

```{r Robustness Check, echo=FALSE, cache=TRUE}
```

None of the other goods appear to have had a significant change in price at the time of the tax abolition. The estimated coefficients are also quite close to 0. To ensure that the estimated models are appropriate, residual graphs and ACFs are also shown.

```{r Resid Plots for Others, echo=FALSE}
```

A few outliers are present in the residual plots, but the plots do not seem to suggest heteroskedasticity. The ACF plots also suggest that the ARIMA errors have adequately incorporated the autocorrelation of prices. The reported p-values can be trusted, thus providing more evidence that the tax abolition is the primary factor for the sudden drop in tampon prices. 
